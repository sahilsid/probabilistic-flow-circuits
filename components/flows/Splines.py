...
class MixtureSpline(ConditionalTransformModule):
    ...
    def __init__(self, n_mixture, input_dim, count_bins, bound=3.0, order='linear'):
        super().__init__()
        ...
        self.unnormalized_widths  = nn.Parameter(torch.randn(self.n_mixture, self.input_dim, self.count_bins))
        self.unnormalized_heights = nn.Parameter(torch.randn(self.n_mixture, self.input_dim, self.count_bins))
        self.unnormalized_derivatives = nn.Parameter(torch.randn(self.n_mixture, self.input_dim, self.count_bins - 1))

        # Rational linear splines have additional lambda parameters
        if self.order == "linear":
            self.unnormalized_lambdas = nn.Parameter(torch.rand(self.n_mixture, self.input_dim, self.count_bins))
        elif self.order != "quadratic":
            raise ValueError(
                "Keyword argument 'order' must be one of ['linear', 'quadratic'], but '{}' was found!".format(
                    self.order))
    ...
    def _params(self, context):
        # widths, unnormalized_widths ~ (input_dim, num_bins)
        w = F.softmax(torch.index_select(self.unnormalized_widths,0,context), dim=-1)
        h = F.softmax(torch.index_select(self.unnormalized_heights,0,context), dim=-1)
        d = F.softplus(torch.index_select(self.unnormalized_derivatives,0,context))
        if self.order == 'linear':
            l = torch.sigmoid(torch.index_select(self.unnormalized_lambdas,0,context))
        else:
            l = None
