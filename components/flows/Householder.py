...
over-parametrization!".format(
    ...
    def _u_unnormed(self, context):
        ...
        if(len(context.shape)==1):
            context = context.unsqueeze(-1).to(torch.float32)
...
class MixtureHouseholder(ConditionalTransformModule):
    r"""
    Represents multiple applications of the Householder bijective transformation
    conditioning on an additional context. A single Householder transformation takes
    the form,
    ...
    def __init__(self, n_mixture, input_dim):
        super().__init__()
        self.nn = DenseNN(1, [64,128,64], param_dims=[input_dim, input_dim])
        self.input_dim = input_dim
        self.n_mixture = n_mixture
        self.W  = nn.Parameter(torch.randn(self.n_mixture, self.input_dim, self.input_dim))
        self.count_transforms = self.input_dim

    def _u_unnormed(self, context):
        u_unnormed =  torch.index_select(self.W,0,context)
        return u_unnormed
...
class ConditionalHouseholder(ConditionalTransformModule):
    ...
    def _u_unnormed(self, context):
        u_unnormed =  self.nn(context).view(-1,self.input_dim,self.input_dim)
        return u_unnormed
